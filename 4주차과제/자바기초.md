...
```
 Test t1 = new Test();
```
익명 함수가 아닌 경우 다음과 같이 인스턴스를 만들고, 여기서의 인스턴스는 클래스 이름인 Test라고 불린다.  
...
```
해당 예시는 익명 함수에 대한 예시로 인스턴스 옆에 블럭이 있다.  
new Test()라는 소스코드가 있어 Test의 이름을 가질 것 같지만 그렇지 않고 이름이 없다.  
